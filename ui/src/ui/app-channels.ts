...
export async function handleWhatsAppStart(host: LynkAIApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: LynkAIApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: LynkAIApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: LynkAIApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: LynkAIApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: LynkAIApp): string {
...
function resolveGatewayHttpAuthHeader(host: LynkAIApp): string | null {
...
function buildGatewayHttpHeaders(host: LynkAIApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: LynkAIApp) {
...
export function handleNostrProfileToggleAdvanced(host: LynkAIApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: LynkAIApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: LynkAIApp) {
  const state = host.nostrProfileFormState;
