...
- With Anthropic API-key auth profiles, LynkAI seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, LynkAI injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
LynkAI exposes dedicated cache-trace diagnostics for embedded agent runs.
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.lynkai/logs/cache-trace.jsonl" # optional
