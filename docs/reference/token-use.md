---
summary: "How LynkAI builds prompt context and reports token usage + costs"
read_when:
...
LynkAI tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

LynkAI assembles its own system prompt on every run. It includes:
...
For images, LynkAI downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from LynkAI session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `lynkai status --usage` and `lynkai channels list` show
...
`cacheWrite`. If pricing is missing, LynkAI shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. LynkAI can
...
Anthropic's 1M context window is currently beta-gated. LynkAI can inject the
...
LynkAI skips the `context-1m-*` beta header because Anthropic currently
