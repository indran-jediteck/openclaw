...
LynkAI integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- LynkAI owns lifecycle when `channels.signal.autoStart=true`.
...
- LynkAI spawns `imsg rpc` as a child process (legacy iMessage integration).
