...
LynkAI features that can generate provider usage or paid API calls.
...
- `lynkai status --usage` and `lynkai channels list` show provider **usage windows**
...
LynkAI can pick up credentials from:
...
`lynkai models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
