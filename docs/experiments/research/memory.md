...
read_when:
  - Designing workspace memory (~/.lynkai/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep LynkAI integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.lynkai/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.lynkai/workspace` as canonical human-readable memory.
...
```
~/.lynkai/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in LynkAI**, but keep a separable core library.

### Why integrate into LynkAI?

- LynkAI already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `lynkai memory recall "…" --k 25 --since 30d`
  - `lynkai memory reflect --since 7d`
...
Pragmatic take for `~/.lynkai/workspace`:
