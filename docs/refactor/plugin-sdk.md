...
- Publish as `lynkai/plugin-sdk` (or export from core under `lynkai/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `LynkAIPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: LynkAIConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: LynkAIConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: LynkAIConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: LynkAIConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: LynkAIConfig): string;
  };
};
```
...
- Introduce `lynkai/plugin-sdk`.
- Add `api.runtime` to `LynkAIPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `lynkaiRuntime: ">=2026.2.0"`).
