...
read_when:
  - You want to place an outbound voice call from LynkAI
...
Voice calls for LynkAI via a plugin. Supports outbound notifications and
...
- Use `lynkai voicecall ...` or the `voice_call` tool
...
```bash
lynkai plugins install ./extensions/voice-call
...
```bash
lynkai voicecall call --to "+15555550123" --message "Hello from LynkAI"
lynkai voicecall continue --call-id <id> --message "Any questions?"
lynkai voicecall speak --call-id <id> --message "One moment"
lynkai voicecall end --call-id <id>
lynkai voicecall status --call-id <id>
lynkai voicecall tail
