...
lynkai system heartbeat last
lynkai logs --follow
lynkai config get agents.defaults.heartbeat
lynkai channels status --probe
```
...
```bash
lynkai config get agents.defaults.heartbeat.activeHours
lynkai config get agents.defaults.heartbeat.activeHours.timezone
lynkai config get agents.defaults.userTimezone || echo "agents.defaults.userTimezone not set"
lynkai cron list
lynkai logs --follow
```
