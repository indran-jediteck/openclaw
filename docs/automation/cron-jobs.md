...
- Jobs persist under `~/.lynkai/cron/` so restarts don’t lose schedules.
...
lynkai cron list
lynkai cron run <job-id>
lynkai cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.lynkai/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `lynkai cron add/edit` or the cron
...
To reduce top-of-hour load spikes across many gateways, LynkAI applies a
...
If `delivery` is omitted for isolated jobs, LynkAI defaults to `announce`.
...
- Job store: `~/.lynkai/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.lynkai/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned by size and line count).
...
- For removed run sessions no longer referenced by the session store, LynkAI archives transcript files and purges old deleted archives on the same retention window.
...
- review growth periodically with `lynkai cron runs` and adjust retention before logs become large
...
lynkai cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
lynkai cron edit <jobId> --agent ops
lynkai cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
lynkai cron run <jobId>
lynkai cron run <jobId> --due
```

Edit an existing job (patch fields):

```bash
lynkai cron edit <jobId> \
...
```bash
lynkai system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`lynkai system event`](/cli/system).
...
- LynkAI applies exponential retry backoff for recurring jobs after consecutive errors:
