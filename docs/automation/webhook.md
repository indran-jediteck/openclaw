...
read_when:
  ...
  - Wiring external systems into LynkAI
title: "Webhooks"
---
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your LynkAI config directory (typically `~/.lynkai/hooks/transforms`).
...
- `lynkai webhooks gmail setup` writes `hooks.gmail` config for `lynkai webhooks gmail run`.
