...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in LynkAI
...
Running LynkAI via Node with `tsx` fails at startup with:

```
[lynkai] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `lynkai status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch lynkai.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node lynkai.mjs status` works on Node 25.
