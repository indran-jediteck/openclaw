...
read_when:
  - You want LynkAI to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables LynkAI to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`lynkai onboard`) and `lynkai channels add` list optional channel plugins.
...
```bash
lynkai plugins install @lynkai/nostr
```

Use a local checkout (dev workflows):

```bash
lynkai plugins install --link <path-to-lynkai>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "lynkai@example.com",
        "lud16": "lynkai@example.com"
      }
    }
  }
}
