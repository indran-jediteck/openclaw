...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects LynkAI to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
lynkai plugins install @lynkai/feishu
```

Local checkout (when running from a git repo):

```bash
lynkai plugins install ./extensions/feishu
```
...
If you just installed LynkAI, run the wizard:
...
2. Configuring app credentials in LynkAI
...
1. You already ran `lynkai channels add` for Feishu
2. The gateway is running (`lynkai gateway status`)
...
```bash
lynkai pairing approve feishu <CODE>
```
...
2. Run `lynkai logs --follow` and look for `chat_id`
...
2. Run `lynkai logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `lynkai gateway status`  | Show gateway status           |
| `lynkai gateway install` | Install/start gateway service |
| `lynkai gateway stop`    | Stop gateway service          |
| `lynkai gateway restart` | Restart gateway service       |
| `lynkai logs --follow`   | Tail gateway logs             |
...
4. Check logs: `lynkai logs --follow`
...
5. Ensure the gateway is running: `lynkai gateway status`
6. Check logs: `lynkai logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.lynkai/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.lynkai/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
