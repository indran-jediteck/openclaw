...
```bash
lynkai plugins install @lynkai/msteams
```

Local checkout (when running from a git repo):

```bash
lynkai plugins install ./extensions/msteams
```

If you choose Teams during configure/onboarding and a git checkout is detected,
LynkAI will offer the local install path automatically.
...
3. Configure LynkAI with those credentials.
...
- Talk to LynkAI via Teams DMs, group chats, or channels.
...
- On startup, LynkAI resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.lynkai/lynkai.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring LynkAI, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                    |
   | ------------------ | -------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `lynkai-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `lynkai plugins install @lynkai/msteams`
   - From a local checkout: `lynkai plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "LynkAI in Teams", "full": "LynkAI in Teams" },
...
LynkAI handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, LynkAI only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/LynkAIShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

LynkAI sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `lynkai message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.lynkai/msteams-polls.json`.
...
```bash
lynkai message send --channel msteams \
...
lynkai message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
lynkai message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
lynkai message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
lynkai message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
