...
read_when:
  - Setting up Twitch chat integration for LynkAI
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. LynkAI connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
lynkai plugins install @lynkai/twitch
```

Local checkout (when running from a git repo):

```bash
lynkai plugins install ./extensions/twitch
```
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "lynkai", // Bot's Twitch account
