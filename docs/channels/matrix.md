...
Matrix is an open, decentralized messaging protocol. LynkAI connects as a Matrix **user**
...
```bash
lynkai plugins install @lynkai/matrix
```

Local checkout (when running from a git repo):

```bash
lynkai plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `lynkai plugins install @lynkai/matrix`
   - From a local checkout: `lynkai plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: LynkAI calls the same
     login endpoint, stores the access token in `~/.lynkai/credentials/matrix/credentials.json`,
...
- On first connection, LynkAI requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  LynkAI logs a warning.
...
`~/.lynkai/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `lynkai pairing list matrix`
  - `lynkai pairing approve matrix <CODE>`
...
- On startup, LynkAI resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
