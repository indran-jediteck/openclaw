...
```bash
lynkai plugins install @lynkai/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
lynkai plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "LynkAI" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `lynkai pairing list nextcloud-talk`
  - `lynkai pairing approve nextcloud-talk <CODE>`
