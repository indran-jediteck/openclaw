---
summary: "Mattermost bot setup and LynkAI config"
read_when:
...
```bash
lynkai plugins install @lynkai/mattermost
```

Local checkout (when running from a git repo):

```bash
lynkai plugins install ./extensions/mattermost
```
...
4. Configure LynkAI and start the gateway.
...
- Approve via:
  - `lynkai pairing list mattermost`
  - `lynkai pairing approve mattermost <CODE>`
...
Use these target formats with `lynkai message send` or cron/webhooks:
