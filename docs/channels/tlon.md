...
Tlon is a decentralized messenger built on Urbit. LynkAI connects to your Urbit ship and can
...
```bash
lynkai plugins install @lynkai/tlon
```

Local checkout (when running from a git repo):

```bash
lynkai plugins install ./extensions/tlon
```
...
By default, LynkAI blocks private/internal hostnames and IP ranges for this plugin (SSRF hardening).
...
Use these with `lynkai message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, LynkAI replies in-thread.
