---
summary: "Synology Chat webhook setup and LynkAI config"
read_when:
  - Setting up Synology Chat with LynkAI
...
```bash
lynkai plugins install ./extensions/synology-chat
```
...
3. Point the outgoing webhook URL to your LynkAI gateway:
   - `https://gateway-host/webhook/synology` by default.
   - Or your custom `channels.synology-chat.webhookPath`.
4. Configure `channels.synology-chat` in LynkAI.
...
- Pairing approvals work with:
  - `lynkai pairing list synology-chat`
  - `lynkai pairing approve synology-chat <CODE>`
...
```bash
lynkai message send --channel synology-chat --target 123456 --text "Hello from LynkAI"
lynkai message send --channel synology-chat --target synology-chat:123456 --text "Hello again"
```
