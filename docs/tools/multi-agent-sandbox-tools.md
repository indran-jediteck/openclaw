...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `lynkai sandbox explain`.
...
Legacy `agent.*` configs are migrated by `lynkai doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=lynkai-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${OPENCLAW_STATE_DIR:-$HOME/.lynkai}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
