---
summary: "Chrome extension: let LynkAI drive your existing Chrome tab"
read_when:
...
The LynkAI Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate lynkai-managed Chrome profile.
...
LynkAI then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the LynkAI release (npm package) as static files. There is no separate “build” step.

After upgrading LynkAI:

- Re-run `lynkai browser extension install` to refresh the installed files under your LynkAI state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (set gateway token once)

LynkAI ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want LynkAI to control.
...
- `ON`: attached; LynkAI can drive that tab.
...
Debugging: `lynkai sandbox explain`
...
`lynkai browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `lynkai browser extension install` first to copy the extension to a stable location under your LynkAI state directory.
...
- **This is not isolated** like the dedicated lynkai-managed profile.
