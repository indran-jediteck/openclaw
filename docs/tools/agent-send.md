---
summary: "Direct `lynkai agent` CLI runs (with optional delivery)"
read_when:
  - Adding or modifying the agent CLI entrypoint
title: "Agent Send"
---

# `lynkai agent` (direct agent runs)

`lynkai agent` runs a single agent turn without needing an inbound chat message.
...
- Optional delivery back to a channel with `--deliver` + `--channel` (target formats match `lynkai message --target`).
...
```bash
lynkai agent --to +15555550123 --message "status update"
lynkai agent --agent ops --message "Summarize logs"
lynkai agent --session-id 1234 --message "Summarize inbox" --thinking medium
lynkai agent --to +15555550123 --message "Trace logs" --verbose on --json
lynkai agent --to +15555550123 --message "Summon reply" --deliver
