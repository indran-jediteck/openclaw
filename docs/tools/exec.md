...
- `gateway`/`node` approvals are controlled by `~/.lynkai/exec-approvals.json`.
...
- `host=sandbox`: runs `sh -lc` (login shell) inside the container, so `/etc/profile` may reset `PATH`.
  LynkAI prepends `env.PATH` after profile sourcing via an internal env var (no shell interpolation);
...
`lynkai security audit` warns when interpreter/runtime `safeBins` entries are missing explicit profiles, and `lynkai doctor --fix` can scaffold missing custom `safeBinProfiles` entries.
