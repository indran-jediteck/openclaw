...
- **gateway host** → `lynkai` process on the gateway machine
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.lynkai/exec-approvals.sock",
...
- allowlist entries live in host-local `~/.lynkai/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `lynkai approvals allowlist ...`).
- `lynkai security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `lynkai doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
`~/.lynkai/exec-approvals.json` directly.

CLI: `lynkai approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
