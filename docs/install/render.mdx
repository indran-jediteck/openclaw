...
Deploy LynkAI on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/lynkai/lynkai)
...
repository configures everything needed to run LynkAI:
...
The web dashboard is available at `https://<your-service>.onrender.com/lynkai`.
...
If you use the original LynkAI repository, Render will not auto-deploy your LynkAI. To update it, run a manual Blueprint sync from the dashboard.
...
For LynkAI, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any LynkAI host.
