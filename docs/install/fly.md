---
title: Fly.io
description: Deploy LynkAI on Fly.io
summary: "Step-by-step Fly.io deployment for LynkAI with persistent storage and HTTPS"
read_when:
  - Deploying LynkAI on Fly.io
  - Setting up Fly volumes, secrets, and first-run config
---

# Fly.io Deployment

**Goal:** LynkAI Gateway running on a [Fly.io](https://fly.io) machine with persistent storage, automatic HTTPS, and Discord/channel access.
...
fly apps create my-lynkai

# Create a persistent volume (1GB is usually enough)
fly volumes create lynkai_data --size 1 --region iad
```
...
- **Prefer env vars over config file** for all API keys and tokens. This keeps secrets out of `lynkai.json` where they could be accidentally exposed or logged.
...
**Note:** With `OPENCLAW_STATE_DIR=/data`, the config path is `/data/lynkai.json`.
...
Or visit `https://my-lynkai.fly.dev/`
...
If using `--allow-unconfigured`, the gateway creates a minimal config. Your custom config at `/data/lynkai.json` should be read on restart.

Verify the config exists:

```bash
fly ssh console --command "cat /data/lynkai.json"
```
...
echo '{"your":"config"}' | fly ssh console -C "tee /data/lynkai.json"

# Or use sftp
fly sftp shell
> put /local/path/config.json /data/lynkai.json
```

**Note:** `fly sftp` may fail if the file already exists. Delete first:

```bash
fly ssh console --command "rm /data/lynkai.json"
```
...
```bash
# List current IPs
fly ips list -a my-lynkai

# Release public IPs
fly ips release <public-ipv4> -a my-lynkai
fly ips release <public-ipv6> -a my-lynkai
...
fly ips allocate-v6 --private -a my-lynkai
```
...
fly proxy 3000:3000 -a my-lynkai
