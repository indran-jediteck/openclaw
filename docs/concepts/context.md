...
read_when:
  - You want to understand what “context” means in LynkAI
...
“Context” is **everything LynkAI sends to the model for a run**. It is bounded by the model’s **context window** (token limit).

Beginner mental model:

- **System prompt** (LynkAI-built): rules, tools, skills list, time/runtime, and injected workspace files.
...
## How LynkAI builds the system prompt

The system prompt is **LynkAI-owned** and rebuilt each run. It includes:
...
By default, LynkAI injects a fixed set of workspace files (if present):
...
Large files are truncated per-file using `agents.defaults.bootstrapMaxChars` (default `20000` chars). LynkAI also enforces a total bootstrap injection cap across files with `agents.defaults.bootstrapTotalMaxChars` (default `150000` chars). `/context` shows **raw vs injected** sizes and whether truncation happened.
