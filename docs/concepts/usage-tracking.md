...
- `/usage cost` in chats: local cost summary aggregated from LynkAI session logs.
- CLI: `lynkai status --usage` prints a full per-provider breakdown.
- CLI: `lynkai channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
