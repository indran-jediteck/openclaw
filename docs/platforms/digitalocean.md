---
summary: "LynkAI on DigitalOcean (simple paid VPS option)"
read_when:
  - Setting up LynkAI on DigitalOcean
  - Looking for cheap VPS hosting for LynkAI
title: "DigitalOcean"
---

# LynkAI on DigitalOcean

## Goal

Run a persistent LynkAI Gateway on DigitalOcean for **$6/month** (or $4/mo with reserved pricing).
...
# Check service
systemctl --user status lynkai-gateway.service

# View logs
journalctl --user -u lynkai-gateway.service -f
```
...
```bash
lynkai config set gateway.bind tailnet
lynkai gateway restart
```
...
```bash
lynkai channels login whatsapp
# Scan QR code
```
...
- `~/.lynkai/workspace/` — workspace (SOUL.md, memory, etc.)
...
journalctl -u lynkai --no-pager -n 50
```
