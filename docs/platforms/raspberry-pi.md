---
summary: "LynkAI on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up LynkAI on a Raspberry Pi
  - Running LynkAI on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# LynkAI on Raspberry Pi

## Goal

Run a persistent, always-on LynkAI Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most LynkAI features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled lynkai

# Enable if not
sudo systemctl enable lynkai

# Start on boot
sudo systemctl start lynkai
```
...
```bash
# Check logs
journalctl -u lynkai --no-pager -n 100

# Common fix: rebuild
cd ~/lynkai  # if using hackable install
npm run build
sudo systemctl restart lynkai
```
