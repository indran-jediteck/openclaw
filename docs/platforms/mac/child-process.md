...
service via the external `lynkai` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.lynkai.gateway`
  (or `ai.lynkai.<profile>` when using `--profile`/`OPENCLAW_PROFILE`; legacy `com.lynkai.*` is supported).
