...
- Same path: run the app from a fixed location (for LynkAI, `dist/LynkAI.app`).
...
```bash
sudo tccutil reset Accessibility ai.lynkai.mac
sudo tccutil reset ScreenCapture ai.lynkai.mac
...
Workaround: move files into the LynkAI workspace (`~/.lynkai/workspace`) if you want to avoid per-folder grants.
