...
```bash
lynkai tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `lynkai tui --deliver`
...
- Check the Gateway logs: `lynkai logs --follow`.
- Confirm the agent can run: `lynkai status` and `lynkai models status`.
...
- No agents in picker: check `lynkai agents list` and your routing config.
