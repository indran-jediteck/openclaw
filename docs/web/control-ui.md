...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/lynkai`)
...
If the page fails to load, start the Gateway first: `lynkai gateway`.
...
you revoke it with `lynkai devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.lynkai/lynkai.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop lynkai`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. LynkAI
...
LynkAI **blocks** Control UI connections without device identity.
