...
你可以在 `~/.lynkai/lynkai.json` 中覆盖此设置：
...
所有日志配置都在 `~/.lynkai/lynkai.json` 的 `logging` 下。
...
- LynkAI 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `lynkai plugins enable diagnostics-otel` 启用插件。
...
- `lynkai.tokens`（计数器，属性：`lynkai.token`、`lynkai.channel`、`lynkai.provider`、`lynkai.model`）
- `lynkai.cost.usd`（计数器，属性：`lynkai.channel`、`lynkai.provider`、`lynkai.model`）
- `lynkai.run.duration_ms`（直方图，属性：`lynkai.channel`、`lynkai.provider`、`lynkai.model`）
- `lynkai.context.tokens`（直方图，属性：`lynkai.context`、`lynkai.channel`、`lynkai.provider`、`lynkai.model`）

消息流：

- `lynkai.webhook.received`（计数器，属性：`lynkai.channel`、`lynkai.webhook`）
- `lynkai.webhook.error`（计数器，属性：`lynkai.channel`、`lynkai.webhook`）
- `lynkai.webhook.duration_ms`（直方图，属性：`lynkai.channel`、`lynkai.webhook`）
- `lynkai.message.queued`（计数器，属性：`lynkai.channel`、`lynkai.source`）
- `lynkai.message.processed`（计数器，属性：`lynkai.channel`、`lynkai.outcome`）
- `lynkai.message.duration_ms`（直方图，属性：`lynkai.channel`、`lynkai.outcome`）

队列 + 会话：

- `lynkai.queue.lane.enqueue`（计数器，属性：`lynkai.lane`）
- `lynkai.queue.lane.dequeue`（计数器，属性：`lynkai.lane`）
- `lynkai.queue.depth`（直方图，属性：`lynkai.lane` 或 `lynkai.channel=heartbeat`）
- `lynkai.queue.wait_ms`（直方图，属性：`lynkai.lane`）
- `lynkai.session.state`（计数器，属性：`lynkai.state`、`lynkai.reason`）
- `lynkai.session.stuck`（计数器，属性：`lynkai.state`）
- `lynkai.session.stuck_age_ms`（直方图，属性：`lynkai.state`）
- `lynkai.run.attempt`（计数器，属性：`lynkai.attempt`）
...
- `lynkai.webhook.error`
  - `lynkai.channel`、`lynkai.webhook`、`lynkai.chatId`、`lynkai.error`
- `lynkai.message.processed`
  - `lynkai.channel`、`lynkai.outcome`、`lynkai.chatId`、`lynkai.messageId`、`lynkai.sessionKey`、`lynkai.sessionId`、`lynkai.reason`
- `lynkai.session.stuck`
  - `lynkai.state`、`lynkai.ageMs`、`lynkai.queueDepth`、`lynkai.sessionKey`、`lynkai.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `lynkai doctor`。
