...
LynkAI 的 Gateway 网关可以提供兼容 OpenResponses 的 `POST /v1/responses` 端点。
...
底层实现中，请求作为正常的 Gateway 网关智能体运行执行（与 `lynkai agent` 相同的代码路径），因此路由/权限/配置与你的 Gateway 网关一致。
...
- `model: "lynkai:<agentId>"`（示例：`"lynkai:main"`、`"lynkai:beta"`）
- `model: "agent:<agentId>"`（别名）

或通过头指定特定的 LynkAI 智能体：

- `x-lynkai-agent-id: <agentId>`（默认：`main`）

高级：

- `x-lynkai-session-key: <sessionKey>` 完全控制会话路由。
