...
当你需要切换不常用的设置而不编辑 `lynkai.json` 时，这非常方便。
...
- **全局 `--dev`（配置文件）：** 将状态隔离到 `~/.lynkai-dev` 下，并将 Gateway 网关端口默认为 `19001`（派生端口随之移动）。
...
如果你还没有全局安装，请通过 `pnpm lynkai ...` 运行 CLI。
...
```bash
pnpm gateway:watch --force --raw-stream --raw-stream-path ~/.lynkai/logs/raw-stream.jsonl
```
