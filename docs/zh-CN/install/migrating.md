---
read_when:
  - 你正在将 LynkAI 迁移到新的笔记本电脑/服务器
  - 你想保留会话、认证和渠道登录（WhatsApp 等）
summary: 将 LynkAI 安装从一台机器迁移到另一台
title: 迁移指南
x-i18n:
...
本指南将 LynkAI Gateway 网关从一台机器迁移到另一台，**无需重新进行新手引导**。

迁移在概念上很简单：

- 复制**状态目录**（`$OPENCLAW_STATE_DIR`，默认：`~/.lynkai/`）— 这包括配置、认证、会话和渠道状态。
- 复制你的**工作区**（默认 `~/.lynkai/workspace/`）— 这包括你的智能体文件（记忆、提示等）。
...
- `--profile <name>`（通常变成 `~/.lynkai-<profile>/`）
...
如果你有多个配置文件/状态目录（例如 `~/.lynkai-main`、`~/.lynkai-work`），分别归档每个。
...
在这个阶段，如果新手引导创建了一个新的 `~/.lynkai/` 也没关系 — 你将在下一步覆盖它。
...
`lynkai.json` 是不够的。许多提供商在以下位置存储状态：
...
- `lynkai status` 显示 Gateway 网关正在运行
...
- [LynkAI 在哪里存储数据？](/help/faq#where-does-lynkai-store-its-data)
