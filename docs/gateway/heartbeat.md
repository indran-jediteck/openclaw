...
- During heartbeat runs, LynkAI treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `lynkai sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, LynkAI skips the heartbeat run entirely (no model call).
...
```bash
lynkai system event --text "Check for urgent follow-ups" --mode now
```
