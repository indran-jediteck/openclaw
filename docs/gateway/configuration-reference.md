...
description: "Complete field-by-field reference for ~/.lynkai/lynkai.json"
summary: "Complete reference for every LynkAI config key, defaults, and channel settings"
read_when:
...
Every field available in `~/.lynkai/lynkai.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — LynkAI uses safe defaults when omitted.
...
- Legacy single-account Baileys auth dir is migrated by `lynkai doctor` into `whatsapp/default`.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["lynkai-dm"] },
      guilds: {
        "123456789012345678": {
...
- LynkAI additionally attempts voice receive recovery by leaving/rejoining a voice session after repeated decrypt failures.
...
Mattermost ships as a plugin: `lynkai plugins install @lynkai/mattermost`.
...
LynkAI spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@lynkai"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `lynkai.json`).
...
Optional repository root shown in the system prompt's Runtime line. If unset, LynkAI auto-detects by walking upward from the workspace.
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, LynkAI assumes `anthropic` (deprecated).
    ...
    defaults: {
      sandbox: {
        ...
        browser: {
          ...
          network: "lynkai-sandbox-browser",
          cdpPort: 9222,
...
- `none`: per-scope sandbox workspace under `~/.lynkai/sandboxes`
...
noVNC observer access uses VNC auth by default and LynkAI emits a short-lived token URL (instead of exposing the password in the shared URL).

- `allowHostControl: false` (default) blocks sandboxed sessions from targeting the host browser.
- `network` defaults to `lynkai-sandbox-browser` (dedicated bridge network). Set to `bridge` only when you explicitly want global bridge connectivity.
    ...
    list: [
      {
        ...
        agentDir: "~/.lynkai/agents/main/agent",
        ...
        groupChat: { mentionPatterns: ["@lynkai"] },
        sandbox: { mode: "off" },
...
- **`parentForkMaxTokens`**: max parent-session `totalTokens` allowed when creating a forked thread session (default `100000`).
  - If parent `totalTokens` is above this value, LynkAI starts a fresh thread session instead of inheriting parent transcript history.
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:lynkai`   | All built-in tools (excludes provider plugins)                                           |
...
LynkAI uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.lynkai/agents/<agentId>/agent/models.json`.
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Shortcut: `lynkai onboard --auth-choice opencode-zen`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `lynkai onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `lynkai onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `lynkai onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `lynkai onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `lynkai onboard --auth-choice minimax-api`.
...
- Loaded from `~/.lynkai/extensions`, `<workspace>/.lynkai/extensions`, plus `plugins.load.paths`.
...
Convenience flags: `--dev` (uses `~/.lynkai-dev` + port `19001`), `--profile <name>` (uses `~/.lynkai-<name>`).
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.lynkai/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-lynkai-token: <token>`.
...
- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__lynkai__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__lynkai__/a2ui/`
...
- Also serves A2UI at `/__lynkai__/a2ui/`.
...
- Hostname defaults to `lynkai`. Override with `OPENCLAW_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.lynkai/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `lynkai dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.lynkai/.env` (neither overrides existing vars).
...
- Legacy OAuth imports from `~/.lynkai/credentials/oauth.json`.
...
- Default log file: `/tmp/lynkai/lynkai-YYYY-MM-DD.log`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `lynkai doctor --fix` can strip unknown keys).
