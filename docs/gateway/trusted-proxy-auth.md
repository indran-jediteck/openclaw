...
read_when:
  - Running LynkAI behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of LynkAI
...
- You run LynkAI behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. LynkAI checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. LynkAI extracts the user identity from the configured header
...
- LynkAI can stay on loopback HTTP behind the proxy.
...
If LynkAI itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://lynkai.example.com
    to: http://lynkai-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
`lynkai security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update LynkAI config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `lynkai security audit` and review findings
