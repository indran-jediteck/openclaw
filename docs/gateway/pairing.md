...
lynkai nodes reject <requestId>
lynkai nodes status
lynkai nodes rename --node <id|name|ip> --name "Living Room iPad"
```
...
Pairing state is stored under the Gateway state directory (default `~/.lynkai`):

- `~/.lynkai/nodes/paired.json`
- `~/.lynkai/nodes/pending.json`
