---
summary: "Use MiniMax M2.1 in LynkAI"
read_when:
  - You want MiniMax models in LynkAI
...
```bash
lynkai plugins enable minimax-portal-auth  # skip if already loaded.
lynkai gateway restart  # restart if gateway is already running
lynkai onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/lynkai/lynkai/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `lynkai.json`:
...
- Use `lynkai models list` and `lynkai models set minimax/MiniMax-M2.1` to switch.
...
- Running `lynkai configure` and selecting **MiniMax M2.1**, or
