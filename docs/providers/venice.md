---
summary: "Use Venice AI privacy-focused models in LynkAI"
read_when:
  - You want privacy-focused inference in LynkAI
...
```bash
lynkai onboard --auth-choice venice-api-key
```
...
```bash
lynkai agent --model venice/llama-3.3-70b --message "Hello, are you working?"
```

## Model Selection

After setup, LynkAI shows all available Venice models. Pick based on your needs:
...
```bash
lynkai models set venice/claude-opus-45
lynkai models set venice/llama-3.3-70b
```

List all available models:

```bash
lynkai models list | grep venice
```

## Configure via `lynkai configure`

1. Run `lynkai configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
LynkAI automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
lynkai agent --model venice/llama-3.3-70b --message "Quick health check"

# Use Claude via Venice (anonymized)
lynkai agent --model venice/claude-opus-45 --message "Summarize this task"

# Use uncensored model
lynkai agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
lynkai agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
lynkai agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `lynkai models list` to see currently available models. Some models may be temporarily offline.
